...
export async function PATCH(
  request: Request,
  {
    ...
    params: Promise<{ companySlug: string; targetCompanyId: string; followUpId: string }>;
  },
) {
  const { companySlug, targetCompanyId, followUpId } = await params;
  ...
  if (ownerId) {
    const membership = await prisma.companyMembership.findFirst({
      where: {
        companyId: existingFollowUp.companyId,
        userId: ownerId,
      },
      select: {
        id: true,
